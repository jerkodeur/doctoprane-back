...
// Fetch all the medications datas (for the patient)
router.get('/:id', (req, res) => {
  ...
  request += ' GROUP BY m.order_name, m.med_name, m.date, m.dosage, m.used, m.morning, m.midday, m.evening, m.night, m.id, d.firstname, d.lastname, d.specialty'
...
// Get the medication datas filter by date asc (for the patient)
router.get('/:id/filter/date/', (req, res) => {
  ...
  request += ' GROUP BY m.order_name, m.med_name, m.date, m.dosage, m.used, m.morning, m.midday, m.evening, m.night, d.firstname, d.lastname, d.specialty'
  connexion.query(request, req.params.id, (err, result) => {
    if (err) {
      return res.status(500).json({
        message: err.message,
...
// Get the medication datas filter by doctor (for the patient)
...
// Get the medication datas filter by specialty (for the patient)
